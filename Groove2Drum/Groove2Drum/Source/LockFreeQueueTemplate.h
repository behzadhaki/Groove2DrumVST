...
private:
    juce::ScopedPointer<juce::AbstractFifo> lockFreeFifo;
    juce::Array<Note> data;

public:

    NoteQueue()
    {
        lockFreeFifo = new juce::AbstractFifo(settings::note_queue_size);

        data.ensureStorageAllocated(settings::note_queue_size);
    ...
    void WriteTo (const Note* writeData, int numTowrite)
    {
        int start1, start2, blockSize1, blockSize2;
    ...
    void ReadFrom(Note* readData, int numToRead)
    {
        int start1, start2, blockSize1, blockSize2;
